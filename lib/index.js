...
function setMouseListener(_ref2) {
  ...
  if (_eventMaps.mouseEventsMap[event] || force && force.forceSetMouse) target.addEventListener(event, handler, listenerOptions);
}
...
function parseKey(key) {
  ...
  var optionsList = { passive: 1, capture: 1, forceSetMouse: 1, forceSetTouch: 1 };
  return {
...
function addListener(target, eventsAndHandlers, pointerOptions) {
  ...
  Object.keys(eventsAndHandlers).forEach(function (key) {
    ...
    var force = _parseKey.force;

    events.forEach(function (event) {
      setListener({ target: target, event: event, handler: handler, listenerOptions: listenerOptions, touchState: touchState, pointerOptions: pointerOptions, force: force });
    });
  });
}